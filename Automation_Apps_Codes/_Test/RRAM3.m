function I = RRAM3(T0,Tf,tp,Vp,V0,Vf)
I=true;
format longE
...
writeline(KLYSM2450,'Voltage_Current_Buffer = buffer.make(10),buffer.STYLE_WRITABLE_FULL')
 ...
 while (realtemptest<Tf) % Heating
        ...
        %str2double(writeread(KLYSM2450,'printbuffer(1,1,Voltage_Current_Buffer.readings)'));
        
        disp('      device temperature:                 real temperature:');
        Y2=['              ',num2str(outputtemptest),'                                ',num2str(realtemptest)];
        disp(Y2)
        disp('             VOLTAGE:                           CURRENT:');
        Y3=['              ',num2str(voltagetest),'                            ',num2str(currenttest)];
        disp(Y3);
